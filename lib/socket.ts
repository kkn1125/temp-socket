...
export async function joinRoom(roomId: string, userId: string, isReconnect: boolean = false): Promise<void> {
...
export async function sendMessage(roomId: string, userId: string, message: string): Promise<void> {
  const sock = await connectSocket();
  if (sock && sock.connected) {
...
export function onRoomUpdate(callback: (data: { roomId: string; participantCount: number }) => void): void {
  if (socket) {
...
export function offRoomUpdate(callback: (data: { roomId: string; participantCount: number }) => void): void {
  if (socket) {
